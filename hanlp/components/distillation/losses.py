...
def kd_mse_loss(logits_S, logits_T, temperature=1):
    ...
    return F.mse_loss(beta_logits_S, beta_logits_T)
...
def kd_ce_loss(logits_S, logits_T, temperature=1):
    ...
    return -(p_T * F.log_softmax(beta_logits_S, dim=-1)).sum(dim=-1).mean()
...
def att_mse_loss(attention_S, attention_T, mask=None):
    ...
    * If the `inputs_mask` is given, masks the positions where ``input_mask==0``.
    ...
    if mask is None:
        ...
        return F.mse_loss(attention_S_select, attention_T_select)
    else:
        ...
        return (
            F.mse_loss(attention_S, attention_T, reduction='none')
            * mask.unsqueeze(-1)
            * mask.unsqueeze(2)
        ).sum() / valid_count
...
def att_ce_loss(attention_S, attention_T, mask=None):
    ...
    * If the `inputs_mask` is given, masks the positions where ``input_mask==0``.
    ...
    if mask is None:
        ...
        return -(
            (probs_T_select * F.log_softmax(attention_S, dim=-1)).sum(dim=-1)
        ).mean()
        ...
        return (
            -(
                (
                    probs_T
                    * F.log_softmax(attention_S, dim=-1)
                    * mask.unsqueeze(2)
                ).sum(dim=-1)
                * mask
            ).sum()
...
def hid_mse_loss(state_S, state_T, mask=None):
    ...
    * If the hidden sizes of student and teacher are different, 'proj' option is required in `inetermediate_matches` to match the dimensions.
    ...
    return (
        F.mse_loss(state_S, state_T, reduction='none') * mask.unsqueeze(-1)
    ).sum() / valid_count
...
def cos_loss(state_S, state_T, mask=None):
    ...
    * If the hidden sizes of student and teacher are different, 'proj' option is required in `inetermediate_matches` to match the dimensions.
    ...
    return F.cosine_embedding_loss(state_S, state_T, target, reduction='mean')
...
def pkd_loss(state_S, state_T, mask=None):
    ...
    * If the hidden sizes of student and teacher are different, 'proj' option is required in `inetermediate_matches` to match the dimensions.
    ...
    return (normed_cls_S - normed_cls_T).pow(2).sum(dim=-1).mean()
...
def mmd_loss(state_S, state_T, mask=None):
    ...
    * If the `inputs_mask` is given, masks the positions where ``input_mask==0``.
    ...
    if mask is None:
        ...
        return (
            F.mse_loss(gram_S, gram_T, reduction='none')
            * mask.unsqueeze(-1)
            * mask.unsqueeze(1)
        ).sum() / valid_count
