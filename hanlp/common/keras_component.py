...
class KerasComponent(Component, ABC):
    ...
    def evaluate(self, input_path: str, save_dir=None, output=False, batch_size=128, logger: logging.Logger = None,
        ...
        if output:
            ...
            if isinstance(output, bool):
                output = f'{os.path.join(save_dir, name)}.predict{ext}'
            elif isinstance(output, str):
                output = output
            else:
                raise RuntimeError(f'output ({repr(output)}) must be of type bool or str')
        timer = Timer()
    ...
    def evaluate_output(self, tst_data, out, num_batches, metrics: List[tf.keras.metrics.Metric]):
        ...
        for idx, batch in enumerate(tst_data):
            ...
            print(f'\r{idx + 1}/{num_batches} {format_metrics(metrics)}', end='')
        print()
    ...
    def build(self, logger, **kwargs):
        ...
        if not isinstance(metrics, list) and isinstance(
    ...
    def fit(self, trn_data, dev_data, save_dir, batch_size, epochs, run_eagerly=False, logger=None, verbose=True,
        ...
        if history and hasattr(history, 'epoch'):
            ...
            logger.info(
                f'Trained {trained_epoch} epochs in {delta_time}, each epoch takes {delta_time / trained_epoch if trained_epoch else delta_time}'
            )
    ...
    def build_callbacks(self, save_dir, logger, **kwargs):
        ...
        if lr_decay_per_epoch := self.config.get('lr_decay_per_epoch', None):
            if learning_rate := self.model.optimizer.get_config().get(
                'learning_rate', None
            ):
                ...
                logger.warning(
                    f'Learning rate decay not supported for optimizer={repr(self.model.optimizer)}'
                )

        if anneal_factor := self.config.get('anneal_factor', None):
            callbacks.append(tf.keras.callbacks.ReduceLROnPlateau(factor=anneal_factor,
                                                                  patience=self.config.get('anneal_patience', 10)))
        if early_stopping_patience := self.config.get(
    ...
    def predict(self, data: Any, batch_size=None, **kwargs):
        ...
        for batch in dataset:
            ...
            inputs = (
                data[num_samples : num_samples + samples_in_batch]
                if data_is_list
                else None
            )

            results.extend(iter(self.predict_batch(batch, inputs=inputs, **kwargs)))
    ...
    def predict_batch(self, batch, inputs=None, **kwargs):
        ...
        yield from self.transform.Y_to_outputs(
            Y, X=X, inputs=inputs, batch=batch, **kwargs
        )
